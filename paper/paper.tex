...
fully nonlinear crest-action conservation equations are mostly steady in the
reference frame of the long waves.
\end{abstract}
...
waves.
The numerical simulations stabilize if the short-wave field is initialized
...
\end{tabular}
\caption{
  Summary of prior and present (this paper) approaches to calculating the
...
in the open ocean, it is a helpful approximation here to isolate the effects
...
unstable growth of wave action in infinite long-wave trains.
...
linearized conservation of wave crests in (\ref{eq:wavenumber}) results in
...
for the wavenumber, except that here we linearize the wave action balance
...
crest and action conservation equations.
(\ref{eq:wavenumber}) and (\ref{eq:wave_action2}) are discretized using
...
\begin{figure}
  \centering
  \includegraphics[width=0.8\textwidth]{figures/fig_modulation_ramp_timeseries.png}
  \caption{
...
\begin{figure}
  \centering
  \includegraphics[width=\textwidth]{figures/inhomogeneity_tendencies.png}
  \caption{
...
This approach, of course, isolates this particular process from others
...
Although in this paper we only explored the wavenumbers 1 and 10 for the long
and short waves, respectively ($k/k_L = 10$), the results are not sensitive
to different values of $k_L$, provided that the amplitude $a_L$ is set such that
the long-wave steepness $\varepsilon_L = a_L / k_L$ is the same.
Numerical simulations with that compare $k/k_L = 10$ and $k/k_L = 100$ show
very little sensitivity $k/k_L$ (not shown), consistent with the results of
\citet{longuet1987propagation}.
...
the solutions are stable only if the short-wave field is initialized from
a prior steady solution of the form of that from \citet{longuet1960changes}.
If the short-wave field is instead initialized from a uniform field, they
found that the short-wave amplitude, and thus steepness as well, grow
unsteadily.
However, in this study we demonstrate that the unsteady steepening of short
...
crests.
In such a hypothetical scenario, sudden forcing on short waves causes the
modulation pattern to detach itself from the long-wave crests and to propagate
at the short-wave group speed.
When this occurs, the modulated short-waves are no longer in a stabilizing
resonance with the surface convergence and divergence induced by the long-wave
orbital velocity, and each long-wave passage further destabilizes the short-wave
...
In the early days of spectral wave modeling, \citet{willebrand1975energy}
examined the effect of a wave spectrum on the short-wave group velocity,
and found the effect of up to several percent modulation.
For a practical implementation in a spectral wave model, this would have to
incur an additional loop over all frequencies for each short-wave frequency.
It was thus not deemed practical to include this effect in wave models at the time.
However, with the recent advances in machine learning and phase-resolving modeling
of short waves, the effect of wave-modulation deserves re-examination.

TODO Implications for spectral wave models and wind input to short waves.
...
\backsection[Data availability statement]{
  ...
  The numerical model is available as a standalone package at
