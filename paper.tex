...
I also thank three anonymous reviewers for their helpful comments.
}
...
\appendix
\section{Wave action conservation requirements}
...
small amplitude (linear) waves propagating in slowly-varying media.
Although \citet{longuet1987propagation} correctly pointed out both requirements
(that the medium is slowly-varying and that the short-wave properties are
approximately homogeneous and stationary) for the wave action conservation to
hold, he incorrectly stated that there is no explicit restriction on the
steepness of the long wave, and that it appears necessary to only assume
small-amplitude short waves and significant separation between $k$ and $k_L$.
We will show that in the general case, both the homogeneity and the stationarity
of a short wave quantity $q$ are dependent on the long-wave steepness
$\varepsilon_L$.
...
This introduces an additional error at the order of $\mathcal{O}(\varepsilon_L^2)$.
...
The nonlinear wave surface velocity is calculated numerically using the method
by \citet{clamond2018accurate}.

\begin{figure}
\centering
\includegraphics[width=0.8\textwidth]{figures/fig_velocity_error_by_phase.pdf}
\caption{
}
...
\begin{figure}
\centering
\includegraphics[width=0.8\textwidth]{figures/fig_relative_velocity_error.pdf}
\caption{
}
