...
forecasting.
In this paper, we revisit the linear (Airy) wave theory and derive higher-order
...
$\partial \sqrt{gk} / \partial x = 1/2 \sqrt{g/k} \partial k / \partial x + 1/2 \sqrt{k/g} \partial g / \partial x$,
we get:
...
\begin{equation}
\label{eq:phi_L}
\phi_L = \dfrac{a_L \omega_L}{k_L} e^{k_L z} \sin{\psi} + \mathcal{O}(\varepsilon_L^4)
\end{equation}
where $z$ is the vertical distance from the mean water level that is positive
upwards.
(\ref{eq:phi_L}) is accurate to the third order in $\varepsilon_L$.
The long-wave induced horizontal and vertical orbital velocities, evaluated at
the wave surface $z = \eta_L$, are:

\begin{equation}
\label{eq:U_L}
U = a_L \omega_L e^{k_L z} \cos{\psi} + \mathcal{O}(\varepsilon_L^4)
\end{equation}

\begin{equation}
\label{eq:w_L}
W = a_L \omega_L e^{k_L z} \sin{\psi} + \mathcal{O}(\varepsilon_L^4)
\end{equation}
...
amplitude of the long waves is of the similar magnitude or large than the
...
\dfrac{\widetilde{a}}{a} = 
  ...
  \approx \frac{(1 + \varepsilon_L)^{0.75}}{(1 - \varepsilon_L)^{0.25}}
...
helpful discussions that improved this paper.
}
